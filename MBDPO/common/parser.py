...
def parse_cfg(cfg: OmegaConf) -> OmegaConf:
    ...
    if cfg.multitask:
        ...
        cfg.work_dir = (
            Path(offline_root) / cfg.task / str(cfg.seed)
            if offline_root
        ...
        cfg.work_dir = (
            Path(online_root) / cfg.task / str(cfg.seed) / cfg.exp_name
            if online_root
            else default_work_dir
        )
    ...
    if cfg.get("model_size", None) is not None:
        ...
        for k, v in MODEL_SIZE[cfg.model_size].items():
            cfg[k] = v
        if cfg.task == "mt30" and cfg.model_size == 21:
    ...
    if cfg.multitask:
        ...
        cfg.task_dim = (
            96 if cfg.task == "mt80" or cfg.get("model_size", 6) in {2, 340} else 64
        )
    else:
        cfg.task_dim = 0
