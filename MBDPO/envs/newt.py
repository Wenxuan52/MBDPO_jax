...
_NEWT_FACTORY_MODULES = {
    "dmcontrol": "envs.dmcontrol",
    "dmcontrol-ext": "envs.dmcontrol",
    "maniskill": "envs.maniskill",
    "metaworld": "envs.metaworld",
    "mujoco": "envs.mujoco",
    "box2d": "envs.box2d",
    "robodesk": "envs.robodesk",
    "ogbench": "envs.ogbench",
    "pygame": "envs.pygame",
    "atari": "envs.atari",
}
_NEWT_MODULE_CACHE = {}
...
def _newt_import_context(newt_root: Path):
    ...
    envs_pkg.__path__ = [str(tdmpc2_root / "envs")]
    envs_pkg.__package__ = "envs"
    sys.modules["envs"] = envs_pkg
    sys.modules.update(_NEWT_MODULE_CACHE)
    ...
    try:
        ...
        for name, module in list(sys.modules.items()):
            if name.startswith("envs."):
                _NEWT_MODULE_CACHE[name] = module
...
def _import_newt_factory_module(newt_root: Path, domain: str):
    module_name = _NEWT_FACTORY_MODULES.get(domain)
    if module_name is None:
        raise ValueError(f"Unsupported Newt/MMBench domain {domain!r}.")
...
def make_child_env(cfg):
    ...
    try:
        module = _import_newt_factory_module(newt_root, child_cfg.domain)
    except ImportError as exc:
        raise ValueError(
            f"Failed to import Newt domain factory for task {child_cfg.task!r} "
            f"in domain {child_cfg.domain!r}: {exc}"
        ) from exc
