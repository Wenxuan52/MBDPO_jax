...
def _load_env_factory(module_name):
    try:
        return importlib.import_module(f"{__name__}.{module_name}").make_env
    except ImportError:
        return missing_dependencies


def _mbdpo_env_factories():
    return [
        _load_env_factory("dmcontrol"),
        _load_env_factory("maniskill"),
        _load_env_factory("metaworld"),
        _load_env_factory("myosuite"),
        _load_env_factory("mujoco"),
    ]
...
def make_env(cfg):
    ...
    if getattr(cfg, "env_source", "mbdpo") != "newt" and cfg.multitask:
        ...
        if getattr(cfg, "env_source", "mbdpo") == "newt":
            ...
            env = None
            for fn in _mbdpo_env_factories():
                try:
                    env = fn(cfg)
                except ValueError:
                    pass
